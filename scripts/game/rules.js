...
export function checkValidMove(board, value, row, col) {
  return matrix.getCol(board, col)[row] === 0;
}

export function checkMoveWinCondition(board, value, row, col) {
...
export function checkDrawCondition(board) {
  // If the entire top row is filled, it is a draw
  return !matrix.getRow(board, 0).includes(0);
}

export function _isConsecutive(arr, value, count) {
